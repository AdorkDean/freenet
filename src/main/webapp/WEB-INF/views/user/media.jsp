				...
				<c:forEach items="${articleList }" var="article" begin="0"
					...
					<div class="Med_tit">
						<div class="Med_list_tit">
							...
							<a style="cursor: pointer;" class="article_title" id="<c:out value="${article.id }"/>"><c:out
									value="${article.title }" /> </a>	
						</div>
						<c:forEach items="${articleList }" var="article1"
							begin="${status.count+(status.count-1) }" end="${status.count+(status.count-1) }">
							<div class="Med_list_tit">
								<div class="Img_show">
									 <img
										src="img/4d3223ae955d43c8a107fc13f57df528.jpg" />
								</div>
								<a style="cursor: pointer;" class="article_title" id="<c:out value="${article1.id }"/>"><c:out
			...
			<div class="Not_Page">
				<ul>
					<c:if test="${totalPages>1 }">
						<li style="border-right: 1px solid #4d8db4;"><a style="cursor: pointer;" id="last_id">《</a></li>
					</c:if>
					<c:forEach begin="1" var="i" end="${totalPages }">
						<li style="border-right: 1px solid #4d8db4;"><a style="cursor: pointer;" id="${i}"
							class="indexbutton"><c:out value="${i}"></c:out></a></li>
					</c:forEach>
					<c:if test="${totalPages>1 }">
						<li style="border-right: 1px solid #4d8db4;"><a style="cursor: pointer;" id="next_id">》</a></li>
					</c:if>
				</ul>
			</div>
		</div>
...
<script type="text/javascript">
	...
	//	alert("heeee");
		var id = $(this).attr("id");
		window.location.href = "user/MediaArticle/get_article?id=" + id;
	})
</script>
<script type="text/javascript">
			...
			.click(
					function() {
						...
						if (l_index > 0) {
							...
							$
									.ajax({
										url : "user/MediaArticle/get_articleListByAjax",
										type : "post",
										dataType : "html",
