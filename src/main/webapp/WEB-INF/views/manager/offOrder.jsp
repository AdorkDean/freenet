						...
						<div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
							<div class="widget am-cf">
								<div class="widget-head am-cf">
									<div class="widget-title am-fl">线下订单</div>
									...
									<div class="Vip_sel">
										...
										<div class="Vip_sele">
											<select id="orderStatus" onchange="findOrder();"data-am-selected>
												...
												<option value="4">买方违约</option>
												<option value="5">卖方违约</option>
												<option value="9">平台介入</option>
												<option value="7">未收到打款</option>
												<option value="8">买方违约申请</option>
												<option value="2">等待买方打款</option>
												<option value="3">等待确认收款</option>
											</select>
										</div>
									</div>
									<form action="manager/order/offlineKeyword" method="post" onsubmit="return checkKeyWord();">
											...
											<tbody>
												<c:forEach var="obj" items="${list}" varStatus="status">
													...
													<c:if test="${obj.status==2}">
														<td><span class="label-warning">等待买方打款</span></td>
													</c:if>
													<c:if test="${obj.status==3}">
														<td><span class="label-warning">等待确认收款</span></td>
													</c:if>
													<c:if test="${obj.status==4}">
														<td><span class="label-danger">买方违约</span></td>
													</c:if>
													<c:if test="${obj.status==5}">
														<td><span class="label-danger">卖方违约</span></td>
													</c:if>
													<c:if test="${obj.status==6}">
														<td><span class="label-danger">已撤单</span></td>
													</c:if>
													<c:if test="${obj.status==7}">
														<td><span class="label-danger">卖方未收到打款</span></td>
													</c:if>
													<c:if test="${obj.status==8}">
														<td><span class="label-danger">买方发起违约</span></td>
													</c:if>
													<td class=""><fmt:formatDate value="${obj.sellCdt}" pattern="yyyy-MM-dd HH:mm" /></td>
													<td>
														<div class="tpl-table-black-operation on-td">
															<c:if test="${obj.status!=0&&obj.status!=6&&obj.status!=7}">
																<a href="manager/order/offline/edit?id=${obj.id}">
																	<i class="am-icon-pencil"></i> 订单详情
																</a>
															</c:if>
															<a href="manager/order/userOffline/list?sellUserId=${obj.sellUserId }">
...
function findOrder(){
	...
	window.location.href="manager/order/offlineStatusSearch?status="+status;
}
